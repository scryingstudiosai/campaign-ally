...
const SpellInputSchema = z.object({
...
const SYSTEM_PROMPT = `Create a balanced D&D spell with clear mechanics.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = SpellInputSchema.safeParse(body);
    ...
    const userPrompt = `Generate a level ${level} spell. Concept: "${concept}".`;
    ...
    if (insertError) {
      return NextResponse.json(
        { success: false, error: 'Failed to save spell' },
        { status: 500 }
      );
    }
